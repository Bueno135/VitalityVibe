...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(mysqli_num_rows($retorno) > 0) {
        ...
        $sql4 = "UPDATE cliente (nome, email, senha, cpf, dt_nasc, sexo, cep, problema_saude, alergias, altura) 
        VALUES ('$nome', '$email', '$senha', '$cpf', '$dt_nasc', '$sexo', '$cep',  '$problema_saude', '$alergias', '$altura')";
        $resultado2 = mysqli_query($conn, $sql4);

        if($resultado1 && $resultado2) {
            echo"Dados editados!!<BR>";
            echo"<a href=entrarcliente.php>Avançar</a>";
        } else {
            echo "Erro ao editar os dados do usuário.";
        }
    }
    $conn->close();
} else {
